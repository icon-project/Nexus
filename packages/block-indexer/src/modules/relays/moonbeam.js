...
function getAddRelayAction(encodedInput) {
  try {
    ...
    logger.error('moonbeam:getAddRelayAction fails to decode input %O', error);
  }
}
...
function getRemoveRelayAction(encodedInput) {
  try {
    ...
    logger.error('moonbeam:getRemoveRelayAction fails to decode input %O', error);
  }
}
...
async function handleAddRelayAction(input, tx, block) {
  for (const address of input.addresses) {
    ...
    if (success) {
      ...
      logger.info('moonbeam:handleAddRelayAction registers relay %s at tx %s', relay.link, tx.hash);
    }
  }
}
...
async function handleRemoveRelayAction(input, tx, block) {
  ...
  if (success) {
    ...
    logger.info('moonbeam:handleRemoveRelayAction unregisters relay %s at tx %s', relay.address, tx.hash);
  }
}

async function handleRelayActions(tx, block) {
  if (bmcMgmtAddress !== tx.to.toLowerCase())
    return false;

  let actionInput = getAddRelayAction(tx.input);

  if (actionInput) {
    logger.info('moonbeam:handleRelayActions addRelay %O at tx %s', actionInput, tx.hash);
  ...
  if (actionInput) {
    logger.info('moonbeam:handleRelayActions removeRelay %O at tx %s', actionInput, tx.hash);
