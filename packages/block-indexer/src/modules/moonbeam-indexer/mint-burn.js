...
async function handleMintBurnEvents(transaction, block) {
  ...
  if (transaction.events[0].data[0].address && process.env.MOONBEAM_BSH_CORE_ADDRESS === transaction.events[0].data[0].address) {
...
async function handleTransferSingleEvent(transferSingle, evmLogEvent, transaction, block) {
  try {
    ...
    logger.info(`moonbeam:handleTransferSingleEvent in tx hash: ${transaction.hash}`);
  }
}
